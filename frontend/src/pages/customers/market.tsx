...
async function transferTicket(tokenId: number, to: string, feePercentage: number) {
      
      const cust_secret_key = Cookies.get("cust_secret_key")!;
      ...
      const transaction = await contract.transferTicket(tokenId, to, feePercentage, {
      value: value,
      });
...
function TicketForSale({ name, place, date, ticketPrice, ticketNumber, tokenId }: TicketProps) {
  ...
  return (
    <div className="ticket">
      ...
      {!purchased && (
        <button onClick={() => transferTicket(tokenId, cust_wallet_address, 2)}>Purchase</button>
      )}
    </div>
  );
}
  ...
  useEffect(() => {
    async function getCollections() {
      ...
      for (let i = 0; i < collectionCount; i++) {
        ...
        console.log("ticket count " + collection.ticketCount + "ticket sold " + collection.ticketSold);
        ...
        if (parseInt(collection.ticketSold) >= parseInt(collection.ticketCount)) {
        soldOutCollections.push({ name: collection.name, tokenIds: [tokenId], ticketSold: collection.ticketSold, ticketCount: collection.ticketCount });
        soldOutTickets.push(ticket);
        } else {
        collections.push({ name: collection.name, tokenIds: [tokenId], ticketSold: collection.ticketSold, ticketCount: collection.ticketCount  });
        tickets.push(ticket);  
      }
