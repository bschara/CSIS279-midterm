...
function createCollection(string memory name, string memory place, string memory date, uint ticketPrice,  
    uint ticketCount, address organizerAddress) public onlyOwner {
    for (uint i = 0; i < ticketCount; i++) {
        TicketData memory newTicket = TicketData(name, place, date, ticketPrice, i + 1, organizerAddress);
    ...
    Collection memory newCollection = Collection(name, _collections[_collectionIdCounter.current()].tokenIds, ticketCount, 0);
    _collections[_collectionIdCounter.current()] = newCollection;
    _collectionIdCounter.increment();
}

function transferTicket(uint256 tokenId, address payable to, uint256 feePercentage) public payable {
    require(_exists(tokenId), "Ticket does not exist");
    require(ownerOf(tokenId) == msg.sender, "Only ticket owner can transfer ticket");
    require(to != address(0), "Cannot transfer to zero address");
    require(msg.value >= _ticketData[tokenId].ticketPrice, "Insufficient payment");
    ...
    address payable organizer = payable(_ticketData[tokenId].organizerAddress);
...
function getTicketData(uint256 tokenId) public view returns (TicketData memory) {
    require(_exists(tokenId), "Ticket does not exist");
...
function getCollectionsByOrganizer(address organizer) public view returns (Collection[] memory) {
    uint256 collectionCount = _collectionIdCounter.current();
    uint256 matchingCollectionCount = 0;
    Collection[] memory matchingCollections = new Collection[](collectionCount);

    for (uint256 i = 0; i < collectionCount; i++) {
        Collection memory collection = _collections[i];
        if (collection.tokenIds.length > 0 && _ticketData[collection.tokenIds[0]].organizerAddress == organizer) {
...
function getCollectionByTokenId(uint256 tokenId) public view returns ( uint) {
